import * as React from 'react';

function SvgCarIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <path
        d="M7.5 16C8.32843 16 9 15.3284 9 14.5C9 13.6716 8.32843 13 7.5 13C6.67157 13 6 13.6716 6 14.5C6 15.3284 6.67157 16 7.5 16Z"
        fill="currentColor"
      />
      <path
        d="M18 14.5C18 15.3284 17.3284 16 16.5 16C15.6716 16 15 15.3284 15 14.5C15 13.6716 15.6716 13 16.5 13C17.3284 13 18 13.6716 18 14.5Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M5.7 4C5.30649 4 4.94956 4.23078 4.78808 4.58964L2.08808 10.5896C2.03002 10.7187 2 10.8585 2 11V18C2 18.0771 2.00873 18.1522 2.02527 18.2244C2.00868 18.3137 2 18.4058 2 18.5V20.5C2 21.3284 2.67157 22 3.5 22C4.32843 22 5 21.3284 5 20.5V19H19V20.5C19 21.3284 19.6716 22 20.5 22C21.3284 22 22 21.3284 22 20.5V18.5C22 18.4058 21.9913 18.3137 21.9747 18.2244C21.9913 18.1522 22 18.0771 22 18V11C22 10.8585 21.97 10.7187 21.9119 10.5896L19.2119 4.58964C19.0504 4.23078 18.6935 4 18.3 4H5.7ZM4.54659 10L6.34659 6H17.6534L19.4534 10H4.54659ZM4 12V17H20V12H4Z"
        fill="currentColor"
      />
    </svg>
  );
}

export default SvgCarIcon;
