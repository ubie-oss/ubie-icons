import * as React from 'react';

function SvgConditionGood1Icon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <path
        d="M9.25 11.5C9.94036 11.5 10.5 10.9404 10.5 10.25C10.5 9.55964 9.94036 9 9.25 9C8.55964 9 8 9.55964 8 10.25C8 10.9404 8.55964 11.5 9.25 11.5Z"
        fill="currentColor"
      />
      <path
        d="M16 10.25C16 10.9404 15.4404 11.5 14.75 11.5C14.0596 11.5 13.5 10.9404 13.5 10.25C13.5 9.55964 14.0596 9 14.75 9C15.4404 9 16 9.55964 16 10.25Z"
        fill="currentColor"
      />
      <path
        d="M10.5548 14.1681C10.0953 13.8617 9.47441 13.9859 9.16806 14.4454C8.8617 14.9049 8.98588 15.5258 9.44541 15.8322C9.9165 16.1462 10.811 16.5001 12.0001 16.5001C13.1892 16.5001 14.0837 16.1462 14.5548 15.8322C15.0143 15.5258 15.1385 14.9049 14.8322 14.4454C14.5258 13.9859 13.9049 13.8617 13.4454 14.1681C13.3165 14.254 12.811 14.5001 12.0001 14.5001C11.1892 14.5001 10.6837 14.254 10.5548 14.1681Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12ZM20 12C20 16.4183 16.4183 20 12 20C7.58172 20 4 16.4183 4 12C4 7.58172 7.58172 4 12 4C16.4183 4 20 7.58172 20 12Z"
        fill="currentColor"
      />
    </svg>
  );
}

export default SvgConditionGood1Icon;
