import * as React from 'react';

function SvgMenuIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <path
        d="M4 9C4.55228 9 5 8.55228 5 8C5 7.44772 4.55228 7 4 7C3.44772 7 3 7.44772 3 8C3 8.55228 3.44772 9 4 9Z"
        fill="currentColor"
      />
      <path
        d="M8 7C7.44772 7 7 7.44772 7 8C7 8.55228 7.44772 9 8 9H20C20.5523 9 21 8.55228 21 8C21 7.44772 20.5523 7 20 7H8Z"
        fill="currentColor"
      />
      <path
        d="M8 11C7.44772 11 7 11.4477 7 12C7 12.5523 7.44772 13 8 13H20C20.5523 13 21 12.5523 21 12C21 11.4477 20.5523 11 20 11H8Z"
        fill="currentColor"
      />
      <path
        d="M7 16C7 15.4477 7.44772 15 8 15H20C20.5523 15 21 15.4477 21 16C21 16.5523 20.5523 17 20 17H8C7.44772 17 7 16.5523 7 16Z"
        fill="currentColor"
      />
      <path
        d="M5 12C5 12.5523 4.55228 13 4 13C3.44772 13 3 12.5523 3 12C3 11.4477 3.44772 11 4 11C4.55228 11 5 11.4477 5 12Z"
        fill="currentColor"
      />
      <path
        d="M4 17C4.55228 17 5 16.5523 5 16C5 15.4477 4.55228 15 4 15C3.44772 15 3 15.4477 3 16C3 16.5523 3.44772 17 4 17Z"
        fill="currentColor"
      />
    </svg>
  );
}

export default SvgMenuIcon;
